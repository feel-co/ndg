...
Deno.test(
  "worker searches the cached index without resending documents",
  () => {
    send({
      type: "init",
      data: { documents: docs, config: { minWordLength: 2 } },
    });
    const out = send({
      messageId: "a",
      type: "search",
...
Deno.test("worker honours a two-character query", () => {
  ...
  assertGreater(
    out[0].data.length,
    0,
    "two-char query should match 'Terminal'",
  );
  assertEquals(out[0].data[0].doc.title, "Terminal");
});

Deno.test(
  "worker keeps two-character queries below min word length searchable",
  () => {
    send({
      type: "init",
      data: { documents: docs, config: { minWordLength: 3 } },
    });
    const out = send({
      messageId: "c",
      type: "search",
