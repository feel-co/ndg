use comrak::{Arena, ComrakOptions, nodes::{AstNode, NodeValue}, parse_document};
...
fn test_shell_prompt_transformation() {
    ...
    let md = "Run `\$ echo hello` for output.";
    let html = render_html_after_transform(md, &transformer);
    assert!(html.contains("<code class=\"terminal\"><span class=\"prompt\">$</span> echo hello</code>"));
}
...
fn test_repl_prompt_transformation() {
    ...
    let html = render_html_after_transform(md, &transformer);
    assert!(html.contains("<code class=\"nix-repl\"><span class=\"prompt\">nix-repl&gt;</span> 1 + 1</code>"));
}
...
fn test_multiple_prompts_and_normal_code() {
    ...
    assert!(html.contains("<code class=\"nix-repl\"><span class=\"prompt\">nix-repl&gt;</span> help</code>"));
...
fn test_prompt_with_leading_and_trailing_whitespace() {
    ...
    assert!(html.contains("<code class=\"terminal\"><span class=\"prompt\">$</span> echo hi</code>"));
}
