...
use ndg_commonmark::{MarkdownOptions, MarkdownProcessor, create_default_manager};
...
fn test_syntastica_backend_directly() {
    ...
    let highlighter =
        SyntasticaHighlighter::new().expect("Failed to create Syntastica highlighter");

    // Test basic highlighting
    let result = highlighter.highlight("fn main() { println!(\"Hello\"); }", "rust", None);
...
fn test_syntax_manager_language_aliases() {
    let manager = create_default_manager().expect("Failed to create syntax manager");
...
fn test_syntax_manager_fallback_behavior() {
    ...
    let result = manager.highlight_code("some random code", "totally-unsupported-language", None);
...
fn test_theme_handling() {
    ...
    if !themes.is_empty() {
        ...
        let result = manager.highlight_code("fn test() {}", "rust", Some(theme_name));
