...
//!
//! This module provides a comprehensive, trait-based architecture for processing
//! Markdown content with support for various extensions and output formats.
//!
//! # Architecture
//!
...
mod tests {
    ...
    fn test_html_escaped_roles() {
        ...
        {
            let result =
                super::extensions::format_role_markup("option", "hjem.users.<name>.enable", None);
            ...
            assert!(result.contains("<code>hjem.users.&lt;name&gt;.enable</code>"));
            // Should have properly formatted option ID in href with preserved special chars
    ...
    fn test_various_role_types_with_html_characters() {
        #[cfg(any(feature = "nixpkgs", feature = "ndg-flavored"))]
        {
            ...
            let command_result = super::extensions::format_role_markup("command", content, None);
            assert!(command_result.contains("&lt;script&gt;"));
            assert!(!command_result.contains("<script>alert"));

            let env_result = super::extensions::format_role_markup("env", content, None);
            assert!(env_result.contains("&lt;script&gt;"));
            assert!(!env_result.contains("<script>alert"));

            let file_result = super::extensions::format_role_markup("file", content, None);
    ...
    fn test_option_role_special_chars_preserved() {
        ...
        {
            let result =
                super::extensions::format_role_markup("option", "services.foo.<bar>.enable", None);
