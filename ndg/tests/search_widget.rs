...
fn test_search_html_escape() {
  ...
  fs::write(&options_file, option_data().to_string()).unwrap();
  ...
  process_options(&config, &options_file).unwrap();
  generate_search_index(&config, &[]).unwrap();

  let search_data =
    fs::read_to_string(output_dir.join("assets").join("search-data.json"))
      .unwrap();
  let search_docs: Vec<serde_json::Value> =
    serde_json::from_str(&search_data).unwrap();

  let options_html =
    fs::read_to_string(output_dir.join("options.html")).unwrap();
    ...
    let search_entry = search_docs
      ...
      .find(|doc| {
        doc
          ...
          .and_then(|v| v.as_str())
          .is_some_and(|title| {
            title.contains(option_key.split('.').next().unwrap())
          })
      })
      .unwrap_or_else(|| panic!("Search entry not found for {option_key}"));

    let title = search_entry.get("title").unwrap().as_str().unwrap();
    assert!(
