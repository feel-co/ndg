...
You can also select a template for a specific page using the `template` field in
its [frontmatter](./FRONTMATTER):
...
- `{{ toc|safe }}` - Table of contents HTML (empty when frontmatter sets
  `toc = false`)
...
When a document contains a TOML frontmatter block, its fields are available via
the `page` object:
...
- `{{ page.extra }}` - Frontmatter extra table; individual keys via
  `{{ page.extra.key }}`
