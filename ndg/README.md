...
documents, manpages, and PDFs for your Nix related projects from Markdown inputs
and Nix module options. It can be considered a more opinionated and more
specialized alternative to MdBook, choosing to focus on Nix-related projects.
While this project is designed for [Hjem](https://hjem.feel-co.org) in mind, it
is perfectly possible to use NDG outside of Nix contexts. All Nix-specific
...
- **Markdown to HTML, Manpage, and PDF conversion** with support for
  Nixpkgs-flavored CommonMark features[^1].
...
Commands:
  ...
  man     Generate manpage from options
  pdf     Generate PDF from options
...
as HTML. Nevertheless, the `man` subcommand allows generating a manpage from
your options:

```bash
$ ndg man -j ./options.json --output-file ./man/project.5 --title "My Project" --section 5
...
NDG can also generate PDF documentation from your `options.json` using the `pdf`
subcommand:

```bash
$ ndg pdf -j ./options.json --output-file ./project-options.pdf --title "My Project" --header "Generated by NDG"
# => This will generate ./project-options.pdf with your module options.
```

The PDF output supports custom header (`-H`) and footer (`-F`) text, and renders
structured content including headings, lists, code blocks, and admonitions.

> [!NOTE]
> PDF generation, like manpages, is limited to `options.json` inputs. Markdown
> to PDF conversion is not currently supported.
